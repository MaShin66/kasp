...
function bbsCheck(frm){
  ...
  if(frm.addinfo1.value == ""){ alert("예약일을 입력하세요."); frm.addinfo1.focus(); return false; }
  if(frm.addinfo2.value == ""){ alert("예약일을 입력하세요."); frm.addinfo2.focus(); return false; }
  if(frm.addinfo3.value == ""){ alert("예약일을 입력하세요."); frm.addinfo3.focus(); return false; }
  if(frm.addinfo4.value == ""){ alert("예약일을 입력하세요."); frm.addinfo4.focus(); return false; }
  if(frm.addinfo5.value == ""){ alert("예약일을 입력하세요."); frm.addinfo5.focus(); return false; }
...
<table width="100%" border="0" cellpadding="5" cellspacing="0">
  ...
  <tr>
    ...
    <td align="left" colspan="3" style="padding-left:10px;"><input name="tphone" value="<?=$tphone?>" type="text" size="20" class="input" /></td>
  </tr>
  <tr>
    <td colspan="4" height="1" bgcolor="#d7d7d7"></td>
  </tr>
  <tr>
    <td align="center" height="30" bgcolor="#f9f9f9" style="padding-left:10px; border-right:1px solid #d7d7d7;"><strong>예약일 *</strong></td>
    <td align="left" colspan="3" style="padding-left:10px;">
    	<?
    	if($addinfo1 == "") $addinfo1 = date('Y');
    	if($addinfo2 == "") $addinfo2 = date('m');
    	if($addinfo3 == "") $addinfo3 = date('d');
    	?>
    	<select name="addinfo1">
    	<? for($ii=2009;$ii<=2020;$ii++){ ?>
    	<option value="<?=$ii?>" <? if($addinfo1 == $ii) echo "selected"; ?>><?=$ii?></option>
    	<? } ?>
   	 	</select>년

   	 	<select name="addinfo2">
    	<? for($ii=01;$ii<=12;$ii++){ ?>
    	<option value="<?=$ii?>" <? if($addinfo2 == $ii) echo "selected"; ?>><?=$ii?></option>
    	<? } ?>
   	 	</select>월

    	<select name="addinfo3">
    	<? for($ii=01;$ii<=31;$ii++){ ?>
    	<option value="<?=$ii?>" <? if($addinfo3 == $ii) echo "selected"; ?>><?=$ii?></option>
    	<? } ?>
   	 	</select>일

    	<select name="addinfo4">
    	<? for($ii=0;$ii<=23;$ii++){ ?>
    	<option value="<?=$ii?>" <? if($addinfo4 == $ii) echo "selected"; ?>><?=$ii?></option>
    	<? } ?>
   	 	</select>시 ~
    	<select name="addinfo5">
    	<? for($ii=0;$ii<=23;$ii++){ ?>
    	<option value="<?=$ii?>" <? if($addinfo5 == $ii) echo "selected"; ?>><?=$ii?></option>
    	<? } ?>
   	 	</select>시
    </td>
  </tr>
  ...
  <tr>
    <td colspan="4" align="center">
			...
			<tr>
			<td valign="top" align="left">
				<input type="checkbox" name="status" value="Y" <?=$status_checked?>> 예약완료
			</td>
			</tr>
			<tr>
			<td align="left" valign="top">
