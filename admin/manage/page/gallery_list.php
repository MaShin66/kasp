...
$sql = "select count(idx) as all_total from dm_gallery";
...
function delConfirm(code){
    if(confirm("삭제 하시겠습니까?\n등록된 게시물 이미지등 모든 데이터가 삭제되니 주의바랍니다.\n삭제된 데이터는 복구되지 않습니다.")){
        document.location = "gallery_save.php?mode=delete&idx=" + code;
    }
}
//-->
</script>
...
<table style="margin-bottom: 5px;" width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td>등록된 갤러리 연도 갯수 : <b><?=$all_total?></b></td>
        <td align="right">
            <img src="../image/btn_add2.gif" style="cursor:hand" onClick="document.location='gallery_input.php?mode=insert';">
        </td>
    </tr>
</table>
...
<table class="t1" width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    while(($row = mysql_fetch_array($result)) && $rows){
        ...
        <tr>
            ...
            <td align="center"><input type="button" class="btn2" onclick="ycommon.winOpen('/admin/manage/bbs/list.php?code=<?=$row['code']?>','bbs_<?=$row['idx']?>')" value="게시물관리"></td>
            <td align="center"><?=($row['show']) ? 'Y' :'N'?></td>
            <td align="center">
                <img src="../image/btn_edit_s.gif" style="cursor:hand" onClick="document.location='gallery_input.php?mode=update&idx=<?=$row['idx']?>&page=<?=$page?>&<?=$param?>';">
