...
<?php
	...
	if(!strcmp($mode, "grpinsert")) {
		...
		$page_grp = explode("\n",$site_info[page_grp]);
		$page_grp_cnt = count($page_grp) - 1;

		$grp_tmp = explode("^", $page_grp[$page_grp_cnt]);

		$idx = $grp_tmp[0] + 1;

		$page_grp_tmp = $site_info[page_grp]."\n".$idx."^".$grpname;

		$sql = "update wiz_siteinfo set page_grp = '".$page_grp_tmp."'";
