...
?>
				<td valign="top" bgcolor="#ffffff" class="verdana">
					<a href="input.php?code=<?=$code?>&wdate=<?=$year?>-<?=$month?>-<?=sprintf('%02d', $tmp_day)?>"><font size="5" color="<?=$date_color?>"><?=$day?></font></a><br>
					<? for($k=0;$k<count($schedule_list[$tmp_day]);$k++){ ?>
					&nbsp;<a href="view.php?code=<?=$code?>&idx=<?=$schedule_list[$tmp_day][$k][idx]?>"><?=$schedule_list[$tmp_day][$k][subject]?></a><br>
					<? } ?>
				</td>
<?
			$day = $tmp_day;
