...
<? for ($i=0; $i<count($list); $i++) { ?>
    <tr height="33" align="center" <?  if ($list[$i][is_notice])  { echo " bgcolor=#f8f9fa "; } ?>> 
        ...
        if ($list[$i][comment_cnt]) 
            echo " <a href=\"{$list[$i][comment_href]}\"><span style='font-size:7pt; color:#fb7a2c;'>{$list[$i][comment_cnt]}</span></a>";
	...
	echo "<img src='$board_skin_path/img/list_down.gif' border='0'>";
	echo "</a>";
	}
	?>	
	</td>
    <td class="text8"><?=$list[$i][datetime2]?></td>
    <? if ($is_good) { ?><td align="center" class="text8"><?=$list[$i][wr_good]?></td><? } ?>
    <? if ($is_nogood) { ?><td align="center" class="text8"><?=$list[$i][wr_nogood]?></td><? } ?>
</tr>
<tr><td colspan=<?=$colspan?> class='bbs_line'></td></tr>
<?}?>

<? if (count($list) == 0) { echo "<tr><td colspan='$colspan' height=100 align=center>게시물이 없습니다.</td></tr>"; } ?>
<tr><td colspan=<?=$colspan?> class='bbs_line'>
</table>
</form>
