...
include "$_SERVER[DOCUMENT_ROOT]/admin2/inc/sch_info.php";
...
if(strcmp($sch_info[imgview], "N")) {
	...
	for($ii = 1; $ii <= 12; $ii++) {
		if(img_type(WIZHOME_PATH."/data/bbs/$code/M".$bbs_row[upfile.$ii])) {
			${upimg.$ii} = "<div align='".$sch_info[img_align]."'><a href=javascript:viewImg('".$bbs_row[upfile.$ii]."');><img src='/admin2/data/bbs/$code/M".$bbs_row[upfile.$ii]."' border='0' name='wiz_target_resize'></a></div>";
			$_ResizeCheck = true;
		}
	}
}
...
if(!check_point($wiz_session[id], $sch_info[down_point])) {
	...
	for($ii = 1; $ii <= 12; $ii++) {
		if($bbs_row[upfile.$ii] != "") ${upfile.$ii}  = "<a href='/admin2/bbs/down.php?code=$code&idx=$idx&no=".$ii."'>".$bbs_row[upfile.$ii._name]."</a>";
	}
}
