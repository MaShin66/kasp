...
echo "<script Language=\"JavaScript\" src=\"/admin2/js/lib.js\"></script>";
...
if($total > 0) {
	...
	while($row = mysql_fetch_array($result)) {
		...
		if($total < 2 && !strcmp($row[gubun], "A")) {
	  	...
	  	if(!empty($row[catimg])) $catname = "<img src='/admin2/data/category/".$code."/".$row[catimg]."' name='c_".$ii."' border=0 id='c_".$ii."' onMouseOver=WIZ_swapImage('c_".$ii."','','/admin2/data/category/".$code."/".$row[catimg_over]."',1) onMouseOut=WIZ_swapImgRestore()>";
	  	else $catname = $row[catname];

	    if($category == $row[idx]) {
	    	if(!empty($row[catimg])) $catname = "<img src='/admin2/data/category/".$code."/".$row[catimg_over]."' name='c_".$ii."' border=0 id='c_".$ii."' onMouseOver=WIZ_swapImage('c_".$ii."','','/admin2/data/category/".$code."/".$row[catimg_over]."',1) onMouseOut=WIZ_swapImgRestore()>";
...
while($row = mysql_fetch_array($result)){
	...
	if($row[caticon] != "") $catname = "<img src='/admin2/data/category/".$code."/".$row[caticon]."' align='absmiddle'>";		// category
