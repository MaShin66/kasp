...
for($ii = 1;$ii <= $bbs_info[upfile];$ii++){
	if(file_exists(WIZHOME_PATH."/data/bbs/".$code."/M".$row[upfile.$ii])) ${upimg_m.$ii} = "/admin2/data/bbs/$code/M".$row[upfile.$ii];							// img
...
function cuImg<?=$row[idx]?>(){
	if(cuimg<?=$row[idx]?> == ""){
		...
		var url = "/admin2/bbs/view_img.php?code=<?=$code?>&img=" + cuimg<?=$row[idx]?>;
