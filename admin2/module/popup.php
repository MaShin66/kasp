...
if(!readCookie('popupDayClose<?=$popup_info[idx]?>')){
	window.open('/admin2/popup/popup.php?idx=<?=$popup_info[idx]?>','popup<?=$popup_info[idx]?>','height=<?=$popup_info[size_y]?>, width=<?=$popup_info[size_x]?>, menubar=no, scrollbars=<?=$popup_info[scroll]?>, resizable=no, toolbar=no, status=no, top=<?=$popup_info[posi_y]?>, left=<?=$popup_info[posi_x]?>');
}
-->
</script>
...
<?
	...
	// 레이어팝업
		...
		if(!${"popupDayClose".$popup_info[idx]}) include "$_SERVER[DOCUMENT_ROOT]/admin2/popup/popup_layer.php";
