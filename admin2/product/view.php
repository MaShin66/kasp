...
include "$_SERVER[DOCUMENT_ROOT]/admin2/inc/prd_info.php";
...
$content = $row[content];
$prdimg = "/admin2/data/product/".$row[prdimg_M1];
$prdname = $row[prdname];
...
for($ii = 1; $ii <= $prdimg_hide_max; $ii++) {
	if(!is_file("$_SERVER[DOCUMENT_ROOT]/admin2/data/product/".${prdimg_S.$ii})){
		${prdimg_hide_start.$ii} = "<!--"; ${prdimg_hide_end.$ii} = "-->";
	}
}
for($ii = 1; $ii <= $prdfile_max; $ii++) {
	if(!is_file("$_SERVER[DOCUMENT_ROOT]/admin2/data/product/".${upfile.$ii})){
		${upfile_hide_start.$ii} = "<!--"; ${upfile_hide_end.$ii} = "-->";
		${upfile.$ii._click} = " onClick=\"alert('첨부파일이 존재하지 않습니다.');\" style=\"cursor:pointer\" ";
	} else {
		${upfile.$ii} = "<a href='/admin2/product/down.php?prdcode=".$prdcode."&no=".$ii."'>".${upfile.$ii._name}."</a>";
		${upfile.$ii._click} = " onClick=\"document.location='/admin2/product/down.php?prdcode=".$prdcode."&no=".$ii."';\">";
	}
}
...
while($cat_row = mysql_fetch_array($result)){
	...
	if(is_file(WIZHOME_PATH."/data/product/".$cat_row[catimg])) $catimg = "<img src='/admin2/data/product/".$cat_row[catimg]."'>";
...
?>
	if(idx == "<?=$ii?>"){
		...
		document.prdimg.src = "/admin2/data/product/<?=$row[prdimg_M.$ii]?>";
	}
<?php
}
?>
...
function prdImg(){
   var url = "/admin2/product/prdimg.php?prdimg=" + prdimg;
...
// 프린트
function prdPrint(){
   var url = "/admin2/product/print.php?prdcode=<?=$prdcode?>";
