...
include "$_SERVER[DOCUMENT_ROOT]/admin/inc/form_info.php";
...
function postSearch(kind){
   var url = "/admin/form/post_search.php?code=<?=$code?>&kind="+kind;
...
while($row = mysql_fetch_array($result)){
	...
	$fname = $row[fname];
	if(img_type(WIZHOME_PATH."/data/form/title/".$row[fimg])) { $fname = "<img src='/admin/data/form/title/$row[fimg]' align='absmiddle'>"; }
