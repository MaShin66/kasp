...
if(!strcmp($stype, "bbs")) {
	...
	while($row = mysql_fetch_array($result)) {
		...
		if($row[caticon] != "") $catname = "<img src='/admin/data/category/".$row[code]."/".$row[caticon]."' align='absmiddle'>";		// category
		...
		if(!empty($row[upfile1])) {
			...
			if(!check_point($wiz_session[id], $row[down_point])) {
				...
				$file_icon = "<a href='/admin/bbs/down.php?code=$row[code]&idx=$row[idx]&no=1'><img src='$skin_dir/image/file.gif' border='0' align='absmiddle'></a>";			// file
			}
		...
		$upimg_l = $row[upfile1];
		if(file_exists(WIZHOME_PATH."/data/bbs/".$row[code]."/S".$row[upfile1])) $upimg_s = "/admin/data/bbs/$row[code]/S".$row[upfile1];							// img
		else $upimg_s = "$skin_dir/image/noimg.gif";

		if(file_exists(WIZHOME_PATH."/data/bbs/".$row[code]."/M".$row[upfile1])) $upimg_m = "/admin/data/bbs/$row[code]/M".$row[upfile1];							// img
