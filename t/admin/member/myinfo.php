...
$action = $ssl."/admin/member/myinfo_save.php";
...
if($my_info[photo] != "" && file_exists(WIZHOME_PATH."/data/member/".$my_info[photo])){
	$photo = "<img src=/admin/data/member/".$my_info[photo]." name='wiz_target_resize'> <input type='checkbox' name='delphoto' value='Y'><font color='red'>삭제</font><br>";
}
if($my_info[icon] != "" && file_exists(WIZHOME_PATH."/data/member/".$my_info[icon])){
	$icon = "<img src=/admin/data/member/".$my_info[icon]." width='".$icon_size."' height='".$icon_size."' align='absmiddle'> <input type='checkbox' name='delicon' value='Y'><font color='red'>삭제</font><br>";
}
      ...
      //if($tmp_flist[$ii] != ""){
         ...
         if($type == "text"){
            ...
            if(!empty($tmp_value[$ii])) {
	            $finput .= " <a href=\"/admin/data/member/".$tmp_value[$ii]."\" target=\"_blank\">".$tmp_value[$ii]."</a> ";
